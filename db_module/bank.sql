...
IN zhname varchar(20), IN class varchar(10), IN manager varchar(20), IN rate decimal(4,2), IN curclass varchar(20),
...
BEGIN
    ...
    if class = '储蓄' then
		update 账户 set 余额=deposite, 开户日期=opendate, 支行名=zhname, 负责人身份证号=manager
        where 账户号=num;
...
BEGIN
	update 支行 set 资产 = 资产 - OLD.余额 + NEW.余额 where 名字 = NEW.支行名;
END$$
