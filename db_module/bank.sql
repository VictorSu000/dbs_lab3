...
-- Table `bank`.`储蓄业务`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bank`.`储蓄业务` ;

CREATE TABLE IF NOT EXISTS `bank`.`储蓄业务` (
  `编号` INT NOT NULL,
  `名字` VARCHAR(20) NOT NULL,
  `金额` DECIMAL(15,2) NOT NULL,
  `日期` DATE NOT NULL,
  PRIMARY KEY (`编号`),
  INDEX `FK1_idx` (`名字` ASC) VISIBLE,
  CONSTRAINT `FK1`
    FOREIGN KEY (`名字`)
    REFERENCES `bank`.`支行` (`名字`)
    ON DELETE NO ACTION
...
USE `bank`$$
DROP TRIGGER IF EXISTS `bank`.`贷款_AFTER_INSERT` $$
USE `bank`$$
CREATE DEFINER = CURRENT_USER TRIGGER `bank`.`贷款_AFTER_INSERT` AFTER INSERT ON `贷款` FOR EACH ROW
BEGIN
	declare bname varchar(20);
    select 支行_名字 into bname from 银行员工 where 身份证号 = NEW.负责人身份证号;
    if bname!=NEW.名字 then
		signal sqlstate '45000'
		set message_text = '该支行中不存在该员工';
	end if;
END$$
...
BEGIN
    ...
    declare bname varchar(20);
    select 支行_名字 into bname from 银行员工 where 身份证号 = NEW.负责人身份证号;
    if bname!=NEW.支行名 then
		signal sqlstate '45000'
		set message_text = '该支行中不存在该员工';
	end if;
    
    select MAX(编号) into n from 储蓄业务;
    
	update 支行 set 资产 = 资产 + NEW.余额 where 名字 = NEW.支行名;
    
    if n is NULL then set n=0;
    end if;
    insert into 储蓄业务 values(n+1,NEW.支行名,NEW.余额,NEW.开户日期);
END$$
...
BEGIN
    ...
    if OLD.余额 != NEW.余额 then
		select MAX(编号) into n from 储蓄业务;
        select date_format(now(),'%Y-%m-%d') into cur_date;
        insert into 储蓄业务 values(n+1,NEW.支行名,convert(NEW.余额-OLD.余额,signed),cur_date);
	end if;
END$$
...
BEGIN
    ...
    select MAX(编号) into n from 储蓄业务;
	select date_format(now(),'%Y-%m-%d') into del_date;
	insert into 储蓄业务 values(n+1,OLD.支行名,-OLD.余额,del_date);
...
START TRANSACTION;
USE `bank`;
INSERT INTO `bank`.`贷款` (`贷款号`, `名字`, `金额`, `状态`, `负责人身份证号`) VALUES ('DK01', '龙兴', 10000, '发放中', '1001');
...
INSERT INTO `bank`.`账户` (`账户号`, `余额`, `开户日期`, `支行名`, `账户类型`, `负责人身份证号`) VALUES ('ZP01', 0, '2018-01-01', '庞德', '支票', '1003');
